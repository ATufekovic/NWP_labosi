    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="bg-gray-200 rounded-md p-2">{{__("dashboard.articleTitle")}}</h2>
                    <div class="grid sm:grid-cols-3">
                        <div class="bg-gray-100 rounded-md p-2 my-1">{{__("dashboard.articleInfo")}}</div>
                        <div class="bg-gray-100 rounded-md p-2 my-1 ml-1">{{__("dashboard.otherUserFilesInfo")}}</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    @if ($files->isEmpty())
                    <p>{{__("dashboard.noFiles")}}</p>
                    @else    
                    <p>{{__("dashboard.yesFiles")}}</p>
                    @endif
                    <div class="grid sm:grid-cols-3 lg:grid-cols-5">
                        @foreach ($files as $file)
                        
                        <div class="bg-gray-100 p-2 border-t-2 border-l-2">
                            <a href="{{url(App::getLocale() . "/viewUserFiles/" . $file->user->name)}}">
                                <p class="truncate"><span class="font-mono text-sm font-light">{{__("files.itemInfoOwner")}}</span> {{$file->user->name}}</p>
                            </a>
                            <hr>
